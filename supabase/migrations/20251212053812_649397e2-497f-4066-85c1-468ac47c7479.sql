-- Drop and recreate the insert_field_repository function to handle array correctly
DROP FUNCTION IF EXISTS insert_field_repository(uuid, jsonb);

CREATE OR REPLACE FUNCTION insert_field_repository(
  p_user_id uuid,
  p_fields jsonb
)
RETURNS jsonb
...
BEGIN
  ...
  LOOP
    INSERT INTO field_repository (
      ...
      COALESCE(v_field->>'effective_from_date', now()::date::text),
      v_field->>'effective_to_date',
