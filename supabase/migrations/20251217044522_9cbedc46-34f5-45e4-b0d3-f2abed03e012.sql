-- Fix: Remove user_id check from update_field_repository to allow shared administrative configuration
-- This enables any administrator to update field coordinates regardless of who created them
...
DECLARE
  v_result jsonb;
BEGIN
  -- Validate that user exists (for audit purposes)
  IF NOT EXISTS (SELECT 1 FROM custom_users WHERE id = p_user_id) THEN
    RAISE EXCEPTION 'Invalid user_id';
  END IF;

  -- Update field without user_id restriction since field definitions are shared administrative configurations
  ...
  SET
    ...
    last_updated_by = p_user_id::text,
    updated_at = now()
  WHERE id = p_field_id
  ...
  IF v_result IS NULL THEN
    RAISE EXCEPTION 'Field not found or update failed for field_id: %', p_field_id;
  END IF;
