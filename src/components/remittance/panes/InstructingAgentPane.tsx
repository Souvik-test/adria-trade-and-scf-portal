...
const InstructingAgentPane: React.FC<InstructingAgentPaneProps> = ({
  ...
  return (
    <CollapsiblePane title="Instructing Agent" isOpen={isOpen} onToggle={onToggle}>
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Agent Name */}
          <div className="space-y-2">
            <Label htmlFor="instgAgtName" className="text-sm">
              Agent Name <span className="text-destructive">*</span>
            </Label>
            <Input
              id="instgAgtName"
              value={data.instgAgtName}
              onChange={(e) => onChange('instgAgtName', e.target.value.slice(0, 140))}
              placeholder="Enter instructing agent name"
              maxLength={140}
              disabled={readOnly}
          ...
          <div className="space-y-2">
            ...
            {!isBicValid ? (
              ...
              <span className="text-xs text-muted-foreground">{data.instgAgtBic.length}/11</span>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {/* Agent Address */}
          <div className="space-y-2">
            <Label htmlFor="instgAgtAddr" className="text-sm">
              Agent Address
            </Label>
            <Input
              id="instgAgtAddr"
              value={data.instgAgtAddr}
              onChange={(e) => onChange('instgAgtAddr', e.target.value.slice(0, 70))}
