...
const SettlementInstructionsPane: React.FC<SettlementInstructionsPaneProps> = ({
  ...
  return (
    <CollapsiblePane title="Settlement Instructions" isOpen={isOpen} onToggle={onToggle}>
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Instruction Code */}
          <div className="space-y-2">
            <Label htmlFor="instrCd" className="text-sm">
              Instruction Code
            </Label>
            <Select
              value={data.instrCd}
              onValueChange={(value) => onChange('instrCd', value)}
              disabled={readOnly}
            >
              <SelectTrigger className={inputClassName}>
                <SelectValue placeholder="Select instruction code" />
              </SelectTrigger>
              <SelectContent>
                {INSTRUCTION_CODE_OPTIONS.map((opt) => (
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="addtlInfo" className="text-sm">
              Additional Information
            </Label>
            <Textarea
              id="addtlInfo"
              value={data.addtlInfo}
              onChange={(e) => onChange('addtlInfo', e.target.value.slice(0, 140))}
              placeholder="Enter additional settlement instructions"
              maxLength={140}
              rows={3}
              disabled={readOnly}
