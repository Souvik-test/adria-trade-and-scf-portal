...
export const saveRemittanceTransaction = async (
  ...
  try {
    ...
    // Convert status to workflow-compatible format
    const workflowStatus = status === 'draft' ? 'draft' : 
                           status === 'Pending Approval' ? `Data Entry Completed-${initiatingChannel}` :
                           status === 'Approved' ? `Approval Completed-${initiatingChannel}` :
                           status === 'Rejected' ? 'Rejected' : status;
    
    // Determine current stage based on status
    const currentStage = status === 'draft' ? 'Data Entry' : 
...
export const approveRemittanceTransaction = async (
  ...
  try {
    // Fetch the transaction to get the channel
    const { data: txnData } = await supabase
      .from('remittance_transactions')
      .select('initiating_channel')
      .eq('id', transactionId)
      .single();
    
    const channel = txnData?.initiating_channel === 'Bank' ? 'Bank' : 'Portal';
    
    const { error } = await supabase
      .from('remittance_transactions')
      .update({
        status: `Approval Completed-${channel}`,
